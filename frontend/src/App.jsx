...
const TABS = [
  { id: "upload",    label: "📄", full: "Upload",    desc: "Index PDFs" },
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50">
      ...
      {/* Indexed files pills — hidden on mobile */}
      {indexedFiles.length > 0 && (
        <div className="hidden sm:flex max-w-4xl mx-auto px-6 pt-3 flex-wrap gap-2">
          {indexedFiles.map((f) => (
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-4">
        <div className="flex gap-1 sm:gap-1.5 bg-white/70 backdrop-blur rounded-2xl shadow-sm border border-white p-1 sm:p-1.5">
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              <span className={`hidden sm:block text-[9px] mt-0.5 ${activeTab === tab.id ? "text-indigo-200" : "text-gray-400"}`}>
                {tab.desc}
              </span>
              <span className={`sm:hidden text-[9px] mt-0.5 ${activeTab === tab.id ? "text-indigo-200" : "text-gray-400"}`}>
                {tab.full}
              </span>
            </button>
          ))}
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        {activeTab === "upload" && (
          <Upload
            onUploadSuccess={() => {
              refreshFiles();
              addToast("PDF indexed successfully!", "success");
              setActiveTab("qna");
            }}
          />
        )}
        {activeTab === "qna"       && <QnA />}
        {activeTab === "quiz"      && <Quiz onScoreSaved={() => addToast("Score saved to dashboard!", "success")} />}
      ...
      <footer className="text-center py-6 text-[10px] text-gray-300">
        AskMyNotes · FastEmbed · Groq Llama 3.1 · ChromaDB · FastAPI
      </footer>
    </div>
  );
}
