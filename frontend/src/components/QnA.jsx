...
function ChatBubble({ entry }) {
  ...
  return (
    <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} gap-1`}>
      ...
      {/* Source citations */}
      {!isUser && entry.sources?.length > 0 && (
        <div className="max-w-[85%] w-full">
          <button
            onClick={() => setShowSources(!showSources)}
            className="text-[11px] text-indigo-400 hover:text-indigo-600 flex items-center gap-1 ml-1"
          >
...
// Typing indicator shown while waiting for answer
...
export default function QnA() {
  const [history, setHistory] = useState([]); // [{ role: "user"|"assistant", content, sources }]
